...
struct	s_card {
	...
	// Constructors
	s_card( ) { eSuit = CS_INVALID ; eValue = CV_INVALID ; } ;
	s_card( const s_card &copy) : eSuit( copy.eSuit), eValue( copy.eValue) { } ; // { eSuit = copy.eSuit ; eValue = copy.eValue ; } ;
	s_card( const CARDSUITS_T eS, CARDVALUES_T eV) { eSuit = eS ; eValue = eV ; } ;

	// Operators
	bool operator!=( const struct s_card &right) const { return( (this->eSuit != right.eSuit) || (this->eValue != right.eValue)) ; } ;
	bool operator==( const struct s_card &right) const { return( (this->eSuit == right.eSuit) && (this->eValue == right.eValue)) ; } ;
...
public:
	...
	virtual bool				HasFoundationPiles( ) const { return( false) ; } ;
	virtual bool				HasStockPile( ) const { return( false) ; } ;
	virtual bool				HasWastePile( ) const { return( false) ; } ;
	virtual bool				IsPlayable( ) const { return( false) ; } ;
	virtual bool				IsWinner( ) const { return( false) ; } ;
 	virtual int					NumCols( ) const { return( 0) ; } ;
	virtual int					NumReserveSpaces( ) const { return( 0) ; } ;
	virtual int					NumRows( ) const { return( 0) ; } ;
	virtual CARD_T				GetReserveCard( const int nCol) const { CARD_T cInvalidCard ; return( cInvalidCard) ; } ;
	virtual CARD_T				GetTableauCardAt( const int nCol, const int nRow) const { CARD_T cInvalidCard ; return( cInvalidCard) ; } ;
	virtual CARDVALUES_T		GetFoundationCardValue( const CARDSUITS_T eSuit) const { return( CV_INVALID) ; } ;

	// Create a new board
	virtual bool				InitRandomBoard( ) { return (false) ; } ;

	// Clear a board
	virtual void				ClearBoard( ) { } ;

	// Copy an existing board
	virtual bool				CopyBoard( const CSolitaireBoard *pBase) { return( false) ; } ;

	// Auto complete
	virtual MOVE_T				AutoComplete( ) const ;

	// Apply a move
	virtual bool				ApplyMove( const MOVE_T cMove) { return( false) ; } ;

	// Run a cheat
	virtual MOVE_T				ApplyCheatAny( ) { MOVE_T cInvalidMove ; cInvalidMove.eMoveFrom = cInvalidMove.eMoveTo = MP_INVALID ; return( cInvalidMove) ; } ;

	// Debugging aids
	virtual void				DumpBoardToDebug( ) { } ;

	// Compress board
	virtual bool				CompressBoard( COMP_BOARD *pData) const { return( false) ; } ;
	virtual bool				RotateAndCompressBoard( COMP_BOARD *pData) const { return( false) ; } ;
	virtual bool				ExpandBoard( const COMP_BOARD *pData) { return( false) ; } ;
...
public:
	...
	// Information about the board
	virtual bool				HasFoundationPiles( ) const { return( true) ; } ;
	virtual bool				HasStockPile( ) const { return( false) ; } ;
	virtual bool				HasWastePile( ) const { return( false) ; } ;
	virtual bool				IsPlayable( ) const { return( m_bIsPlayable) ; } ;
 	virtual bool				IsWinner( ) const ;
	virtual int					NumCols( ) const { return( NUM_COLS_ALLOCATED) ; } ;
	virtual int					NumReserveSpaces( ) const { return( 4) ; } ;
	virtual int					NumRows( ) const { return( NUM_ROWS_ALLOCATED - 1) ; } ;
